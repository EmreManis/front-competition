...
const SearchArea = () => {
  return (
    <form className="flex flex-col md:flex-row">
      ...
      <div className={`flex border borderRadius my-2 md:my-0 ${classes.inpHeight} ${classes.inpWidthTarih} mr-2.5 pl-6`}>
        <div className="w-1/4 flex items-center">
          <Image src={calendarIcon} alt="tatil konaklama tarihi" width={22} height={22}/>
        </div>
        <BasicDateRangePicker />
      </div>
      <div className={`flex border borderRadius mb-4 md:mb-0 ${classes.inpHeight} ${classes.inpWidthKisi} mr-2.5`}>
          <PersonNum />
      </div>
      <button className={`${classes.buttonArama} ${classes.inpHeight} ${classes.buttonWidth} whiteTone`}>Otel Ara</button>
    </form>
  );
};
